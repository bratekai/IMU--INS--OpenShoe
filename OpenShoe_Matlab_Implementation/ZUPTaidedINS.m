...
% quat.
[x_h(1:9,1) quat]=init_Nav_eq(u);  % Subfunction located further down in the file.
...
for k=2:N
    ...
    % sensor errors  
    u_h=comp_imu_errors(u(:,k),x_h(:,k-1)); % Subfunction located further down in the file.

    
    % Update the navigation equations.   
    [x_h(:,k) quat]=Navigation_equations(x_h(:,k-1),u_h,quat); % Subfunction located further down in the file.

    
    % Update state transition matrix
    [F G]=state_matrix(quat,u_h); % Subfunction located further down in the file.
